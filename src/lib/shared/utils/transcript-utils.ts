...
export function extractSystemPrompt(
  ...
  if (transcript?.metadata?.target_system_prompt) {
    return transcript.metadata.target_system_prompt;
  }

  // Check if events exist and is iterable
  if (!transcript?.events || !Array.isArray(transcript.events)) {
    console.warn(
      "extractSystemPrompt: transcript.events is not a valid array:",
      transcript?.events,
    );
    return undefined;
  }
  ...
  for (const event of transcript.events) {
    if (event.type === "transcript_event") {
      if (
        ...
        event.edit.message?.role === "system"
      ) {
...
export function extractTargetModel(transcript: Transcript): string {
  ...
  if (
    transcript.metadata.target_model &&
    transcript.metadata.target_model !== "unknown"
  ) {
 ...
 */
export function collectScoreDescriptions(
  transcripts: { scoreDescriptions?: Record<string, string> }[],
): Record<string, string> {
  const allDescriptions: Record<string, string> = {};

  for (const transcript of transcripts) {
    if (transcript.scoreDescriptions) {
      for (const [scoreName, description] of Object.entries(
        transcript.scoreDescriptions,
      )) {
...
export function createTranscriptDisplay(
  ...
  const transcriptNumber = fileName.endsWith(".json")
    ? fileName.slice(0, -5)
    : fileName;
  ...
  return {
    ...
    summary:
      transcript.metadata.judge_output?.summary ||
    ...
    justification:
      transcript.metadata.judge_output?.justification ||
